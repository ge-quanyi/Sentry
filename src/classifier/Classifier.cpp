...
void Classifier::lodeSample() {
    ...
    for(int p = 0; p < kClassNum; p++){
        ...
        for(int i = 0; i < img_sum; i++){
            ...
            cv::Mat ROI1 = roi_img_(cv::Rect(0,0,20,55));
            cv::Mat ROI2 = roi_img_(cv::Rect(110,0,20,55));
            ROI1 = cv::Scalar(0,0,0);
            ROI2 = cv::Scalar(0,0,0);
            cv::cvtColor(roi_img_,roi_img_,CV_BGR2GRAY);
            threshold(roi_img_, sample_gray_, 0 ,255,cv::THRESH_OTSU);

            cv::Mat trainingImg_ = cv::Mat(cv::Size(28,28), CV_8UC1);
